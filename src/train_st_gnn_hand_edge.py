"""
train_st_gnn_hand_edge.py  –  PI-ST-GNN Phase 2: HAND dynamic topology
=======================================================================
Architecture
------------
  Extends Phase 1 with structurally new cross-tributary edges that activate
  when observed stage exceeds a precomputed HAND threshold.
...
Prerequisites
-------------
  Run precompute_hand_edges.py once before training:
    python src/precompute_hand_edges.py
  This writes dataset/graph/hand_edges.npz.

Physical basis
--------------
  HAND (Nobre et al. 2011, J. Hydrol.) measures terrain height above nearest
  drainage. Minimum HAND along the overland path between two nodes = stage
  at which floodplain inundation connects their drainage basins laterally.
  5 km search distance: Godbout et al. (2019); Zheng et al. (2018);
  Lee tributary spacing ~3-5 km (Irish Examiner 2024).

Checkpoint directory: checkpoints/st_gnn_hand_edge/
"""
...
def train(logger, seed, t_in, t_out, max_epochs, base_dir = None):
    ...
    if not HAND_EDGES_PATH.exists():
        logger.error(
            "HAND edges not found at %s.\n"
            "Run precompute_hand_edges.py first:\n"
            "  python src/precompute_hand_edges.py",
            HAND_EDGES_PATH,
        )
    ...
    model = STGNNHANDEdge(
        f_dyn=F,
        f_static=f_static,
        f_edge=f_edge + 1,     # 4 static + 1 dynamic (conductance / activation)
        hand_src=hand_data["src"],
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                "hparams": {
                    ...
                    "sar_emb_dim": SAR_EMB_DIM if use_sar_run else 0,
                    "n_hand_edges": len(hand_data["src"]),
                    "hand_max_dist_km": 5.0,
                    "fno_width": FNO_WIDTH,
