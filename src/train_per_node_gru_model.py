"""
train_per_node_gru_model.py  –  Per-node GRU baseline for Lee catchment flood forecasting
==========================================================================================
Architecture:
  1. Node embedding   Linear([F_dyn + F_static]) → hidden_dim
  2. Temporal         GRU (GRU_LAYERS layers) over the T_in-step sequence
  3. Output           Linear head → T_out step forecast per node

  NOTE: This is a non-spatial baseline. No graph message passing is used.
  Each gauge node is modelled independently. It serves as Baseline A in the
...
def train(logger, seed, t_in, t_out, max_epochs, base_dir = None):
    ...
    # ── Load graph (node_attr only — GRU has no message passing) ──────
    ...
    model = compile_model(model, tag="gru", logger=logger)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                "hparams": {
                    "t_in": t_in, "t_out": t_out,
                    "hidden": HIDDEN_DIM, "gru_layers": GRU_LAYERS,
