...
def panel_s2(ax, window_idx: int = 0):
    ...
    hw = meta["headwater_nodes"][0]
    t = _time_axis(T_WINDOW)
    ax.plot(t, y[:, hw], color=COLOR_PRIMARY, lw=1.6, label="True stage (unobserved after failure)")
    obs = y[:, hw].copy()
    t_fail = meta["t_failure_step"]
    obs[t_fail:] = obs[t_fail - 1]   # what the model actually SEES: frozen input
    ax.plot(t, obs, color="gray", lw=1.4, ls=":", label="Frozen sensor reading (model input)")
    _mark_event(ax, t_fail, "Gauge fails")
...
def panel_s4(ax, window_idx: int = 0):
    ...
    # excess_threshold/sat_start/sat_rate_per_step were added to meta
    # alongside the sat_breakthrough_step timing fix -- fall back to the
    # same defaults scenario_generator.py itself uses if this scenario
    # was generated by an older version of the script (recognisable by
    # sat_breakthrough_step == 40, the old buggy T_IN+8 value, rather
    # than the corrected T_IN+14 == 46).
    excess_threshold = meta.get("excess_threshold", 0.75)
    if "sat_start" not in meta:
        print("  [warn] S4 scenario_meta.json predates the sat_breakthrough_step "
              "fix (sat_breakthrough_step=40, not 46) -- regenerate with "
              "scenario_generator.py --scenario S4 for correct results.")
    t = _time_axis(T_WINDOW)
    ax2 = ax.twinx()
...
def panel_s5(ax, window_idx: int = 0):
    ...
    ax.set_title(
        f"S5 — SpatialGradient\ngradient range {meta['gradient_range'][0]:.2f}"
        f"-{meta['gradient_range'][1]:.2f}", fontsize=9.5, fontweight="bold")
    ax.legend(fontsize=7, loc="upper right")
    ax.set_ylabel("Stage (m)")
