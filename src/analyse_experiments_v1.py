...
def plot_dfc_vs_best_baseline(node_df: pd.DataFrame, horizon: int = 4) -> None:
    ...
    best_nse = None
    for m in ["st_gnn_hand_edge", "st_gnn_dyn_edge", "st_gnn"]:
...
def plot_flood_acc_comparison(global_df: pd.DataFrame) -> None:
    ...
    if "flood_acc" not in global_df.columns:
        return
    # Both DFC-GNN variants have auxiliary flood heads
    flood_models = [(m, MODEL_LABELS.get(m, m), MODEL_COLORS.get(m, "#888888"))
                    for m in ("dfc_gnn", "dfc_gnn_unified")
                    if m in global_df["model"].unique()]
    flood_models = [(m, lbl, clr) for m, lbl, clr in flood_models
                    if not global_df[global_df.model == m].flood_acc.isna().all()]
    if not flood_models:
        print("  [skip] flood_acc not yet available")
        return

    n_models = len(flood_models)
    ...
    for (m, lbl, clr), offset in zip(flood_models, offsets):
        sub = global_df[global_df.model == m]
        means = [sub[sub.horizon == hz]["flood_acc"].mean() for hz in HORIZONS]
        stds  = [sub[sub.horizon == hz]["flood_acc"].std(ddof=1)
                 if len(sub[sub.horizon == hz]) > 1 else 0 for hz in HORIZONS]
        ax.bar(x + offset, means, width=width, color=clr,
    ...
    ax.set_title("Auxiliary flood head accuracy — DFC-GNN and PC-DFC-GNN\n"
