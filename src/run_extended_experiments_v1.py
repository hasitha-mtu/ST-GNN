    ...
    for seed in SEEDS:
        ...
        for t_out in TRACK_A_T_OUT:
            for tag, fn in train_fns.items():
                ...
                # Standard checkpoint path: BASE_DIR/checkpoints/{tag}/{seed}/{t_out}
                # T_out = 24,48 do not conflict with existing 4,12,16 — no base_dir needed.
                ckpt_exists = (BASE_DIR / "checkpoints" / tag / str(seed)
                               / str(t_out) / "best_model.pt").exists()
                status = "SKIP — checkpoint exists" if ckpt_exists else "training"
                print(f"\n[Track A  {done}/{total}]  {tag}  seed={seed}  "
                      f"t_in={TRACK_A_T_IN}  t_out={t_out}  ({t_out*15//60}hr)  [{status}]")
                if not dry_run and not ckpt_exists:
...
def _run_tin_ablation_model(fn_train, tag: str, logger,
    ...
    Final checkpoint dir: checkpoints/ablation_tin/{tag}/{seed}/{t_in}/

    Strategy: patch module.BASE_DIR to a staging directory so that
    training scripts write to STAGING/checkpoints/{tag}/{seed}/{T_out}.
    After training, move the result to the final ablation path.

    This works because PROC_DIR and GRAPH_DIR are import-time constants
    in each training module — only checkpoint path uses BASE_DIR at
    call time, so patching BASE_DIR only redirects checkpoint writes.
    """
    import shutil

    # Final destination
    final_dir = (BASE_DIR / "checkpoints" / "ablation_tin"
                 / tag / str(seed) / str(t_in))
    if final_dir.exists():
        print(f"  [SKIP] {tag}/seed{seed}/t_in{t_in} — checkpoint exists")
        return

    # Staging directory — isolated per (tag, seed, t_in) to avoid conflicts
    staging = BASE_DIR / "_ablation_staging" / f"{tag}_{seed}_{t_in}"
    staging.mkdir(parents=True, exist_ok=True)

    # Source of trained checkpoint inside staging
    staged_ckpt = staging / "checkpoints" / tag / str(seed) / str(TRACK_B_T_OUT)

    if dry_run:
        print(f"  [DRY RUN] would train {tag} seed={seed} "
              f"t_in={t_in} → {final_dir}")
        return

    # Patch module BASE_DIR to staging — only checkpoint path is affected
    module = sys.modules.get(fn_train.__module__)
    if module is None:
        print(f"  [skip — cannot find module for {tag}]")
        return

    orig_base = module.BASE_DIR
    module.BASE_DIR = staging
    try:
        fn_train(logger, seed, t_in, TRACK_B_T_OUT, MAX_EPOCHS)
    finally:
        module.BASE_DIR = orig_base  # always restore even on failure

    # Move staged checkpoint to final ablation location
    if staged_ckpt.exists():
        final_dir.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(staged_ckpt), str(final_dir))
        print(f"  Moved checkpoint → {final_dir.relative_to(BASE_DIR)}")
    else:
        print(f"  WARNING: expected checkpoint not found at {staged_ckpt}")
...
def run_track_c(logger, dry_run: bool = False):
    ...
    for seed in SEEDS:
        ...
        for tag, fn in benchmark_fns.items():
            ...
            ckpt_exists = (BASE_DIR / "checkpoints" / tag / str(seed)
                           / str(TRACK_C_T_OUT) / "best_model.pt").exists()
            status = "SKIP — checkpoint exists" if ckpt_exists else "training"
            print(f"\n[Track C  {done}/{total}]  {tag}  seed={seed}  "
                  f"t_in={TRACK_C_T_IN}  t_out={TRACK_C_T_OUT}  (24 hr)  [{status}]")
            if not dry_run and not ckpt_exists:
