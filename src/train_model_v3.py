"""
train_model.py  v3  –  ST-GNN flood forecasting for the Lee catchment
======================================================================
Architecture:
...
def train_epoch(model, loader, optimiser, edge_index, edge_attr,
    ...
    for x_seq, y_seq, mask in loader:
        ...
        loss = masked_mse(delta_pred, delta_target, mask)
        loss.backward()
...
def train(logger):
    ...
    with torch.no_grad():
        ...
        for x_seq, y_seq, mask in test_loader:
            ...
            delta_pred   = model(x_seq, node_attr, edge_index, edge_attr)
            delta_losses.append(
                masked_mse(delta_pred, delta_target, mask_d).item()
            )
