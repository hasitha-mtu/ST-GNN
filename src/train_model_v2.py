...
def train_epoch(model, loader, optimiser, edge_index, edge_attr,
    ...
    for x_seq, y_seq, mask in loader:
        ...
        loss = masked_mse_horizon_weighted(pred, y_seq, mask)
        loss.backward()
...
def eval_epoch(model, loader, edge_index, edge_attr, node_attr):
    ...
    for x_seq, y_seq, mask in loader:
        ...
        pred = model(x_seq, node_attr, edge_index, edge_attr)
        total_loss += masked_mse_horizon_weighted(pred, y_seq, mask).item()
...
def train(logger):
    logger.info("============================Training model v1(Absolute Prediction)=====================================")
    ...
    # Aggregate metrics
    test_loss = masked_mse_horizon_weighted(cat_pred.to(DEVICE),
                           cat_tgt.to(DEVICE),
