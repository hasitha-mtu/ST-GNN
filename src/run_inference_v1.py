...
def compute_extended_metrics(
    pred:      np.ndarray,   # [T_test, N] absolute stage at step T_out-1
...
def infer_one(ckpt_dir: Path, device: torch.device) -> np.ndarray:
    ...
    Returns and saves absolute stage predictions [T_test, N] at the model's
    designed forecast horizon (step T_out-1), not step 0 (15-min ahead).
