...
def visualise_graph(stations: list[dict], edge_index: np.ndarray, edge_attr: np.ndarray,
    ...
    for ax_i, ax in enumerate(axes):
        ...
        for j in range(edge_index.shape[1]):
            ...
            if ax_i == 1:
                mx = (s['lon'] + d['lon']) / 2
                my = (s['lat'] + d['lat']) / 2
                ax.text(mx, my, f"{edge_attr[j,0]:.1f}", fontsize=10, ha='center', color='darkblue')
        ...
        for s in stations:
            if s.get('is_reservoir'):
                ...
                c = color_map['normal']
            ax.scatter(s['lon'], s['lat'], s=65, color=c, zorder=8)
            if ax_i == 0:
                ax.text(s['lon'] + 0.004, s['lat'], s['name'], fontsize=10)

        ax.set_title(['Station labels', 'Edge distances (km)'][ax_i], fontsize=12)
    ...
    fig.suptitle(f'Lee Catchment GNN Graph  ({len(stations)} nodes, {edge_index.shape[1]} edges)\n'
                 f'Edge distances from OPW RiverNetwork',
                 fontsize=12)
